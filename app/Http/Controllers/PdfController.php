...
class PdfController extends Controller
{
    ...
    public function generateAllUsersPdf()
    {
        $users = User::withCount('clocks')->get();

        if ($users->isEmpty()) {
            return back()->with('error', 'Nenhum usuário encontrado para gerar o PDF.');
        }

        $pdf = Pdf::loadView('pdf.all-users', [
            'users' => $users
        ])->setPaper('a4', 'portrait');

        return $pdf->stream('relatorio_usuarios_' . now()->format('Y-m-d') . '.pdf');
    }
}
